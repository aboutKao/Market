...
class MUsure {
    ...
    class func registerUserWithWith(email: String, password: String, completion: @escaping (_ error: Error?) -> Void) {
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            completion(error)
            if error == nil {
                //傳送email驗證
                authDataResult!.user.sendEmailVerification { (error) in
                    print("使用者email驗證錯誤", error?.localizedDescription)
                }
            }
        }
    }
    ...
    class func resendVerificationonEmail(email: String, completion: @escaping (_ error: Error?) -> Void) {
        Auth.auth().currentUser?.reload(completion: { (error) in
            Auth.auth().currentUser?.sendEmailVerification(completion: { (error) in
                print("從新發送email錯誤", error!.localizedDescription)
                completion(error)
            })
        })
